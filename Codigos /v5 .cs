...
class Tablero
{
    ...
    public void Mover()
    {
        ...
        if (piezaOrigen == 1 || piezaOrigen == 4)
        {
            bool valido = ValidarRey(filaOrigen, columnaOrigen, filaDestino, columnaDestino);
        ...
        if (piezaOrigen == 2 || piezaOrigen == 5)
        {
            if (!ValidarTorre(filaOrigen, columnaOrigen, filaDestino, columnaDestino))
                return;
        }
        if (piezaOrigen == 3 || piezaOrigen == 6)
        {
            if (!ValidarSoldado(piezaOrigen, filaOrigen, filaDestino, columnaOrigen, columnaDestino))
                return;
        }
        ...
        if (piezaDestino != 0)
        {
            if ((piezaOrigen >= 1 && piezaOrigen <= 3 && piezaDestino >= 1 && piezaDestino <= 3) ||
                (piezaOrigen >= 4 && piezaOrigen <= 6 && piezaDestino >= 4 && piezaDestino <= 6))
            {
