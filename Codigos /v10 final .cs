...
class Tablero
{
    ...
    public bool ValidarSoldado(int piezaActual, int filaOrigen, int filaDestino, int columnaOrigen, int columnaDestino)
    {
        if (columnaDestino == columnaOrigen)
        {
            if (pieza[filaDestino, columnaDestino] != 0)
            {
                Console.WriteLine("Hay una pieza bloqueando el paso");
                return false;
            }

            if (piezaActual == 3 && filaDestino == filaOrigen - 1)
                return true;

            if (piezaActual == 6 && filaDestino == filaOrigen + 1)
                return true;
        ...
        if (columnaDestino == columnaOrigen + 1 || columnaDestino == columnaOrigen - 1)
        {
            if (pieza[filaDestino, columnaDestino] == 0)
            {
                Console.WriteLine("No hay pieza para atacar");
                return false;
            }
    ...
    public bool ValidarCaminoTorre(int filaOrigen, int columnaOrigen, int filaDestino, int columnaDestino)
    {
        if (filaOrigen == filaDestino)
        {
            ...
            {
                inicio = columnaOrigen + 1;
                fin = columnaDestino;
            }
            else
            {
                inicio = columnaDestino + 1;
                fin = columnaOrigen;
            }
    ...
    {
        bool reyJugador1 = false;
        bool reyJugador2 = false;
        ...
        for (int i = 0; i < 8; i++)
        {
            for (int j = 0; j < 8; j++)
            {
                if (pieza[i, j] >= 1 && pieza[i, j] <= 3)
                {
                    piezasJugador1++;
                }

                if (pieza[i, j] >= 4 && pieza[i, j] <= 6)
                {
                    piezasJugador2++;
                }
        ...
        if (reyJugador1 == false || piezasJugador1 == 0)
        {
            Console.WriteLine("El Jugador 2 ha ganado");

            puntajeMasAlto++;

            Console.WriteLine("Puntaje más alto: " + puntajeMasAlto);

            return true;
        }

        if (reyJugador2 == false || piezasJugador2 == 0)
        {
        ...
        do
        {
            ...
            switch (opcion)
            {
                ...
                case 2:
                    ...
                    Console.WriteLine("cada pieza eliminada vale 10 puntos  ");
                    Console.WriteLine("eliminar el rey vale 50 puntos adicionales ");
                    Console.WriteLine("Rey: puede moverse una sola casilla en cualquier dirección (horizontal, vertical o diagonal).  ");
                    Console.WriteLine("Torre: puede moverse en línea recta (horizontal o vertical), recorriendo varias casillas si el camino está libre. No puede saltar piezas. ");
                    Console.WriteLine("Soldado:  no avanza una casilla hacia adelante  no ataca en diagonal  no puede retroceder ");
                ...
                case 3:
                    ...
                    Console.WriteLine("Puntaje más alto: " + tablero.puntajeMasAlto);
